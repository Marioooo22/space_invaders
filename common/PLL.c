...
#define SYSCTL_RCC_XTAL_FULL      0x000007C0  // Crystal Value
...
#define SYSCTL_RCC2_OSCSRC2_FULL  0x00000070  // Oscillator Source 2
...
void PLL_Init(void){
  ...
  //    such as 400 MHz PLL and non-integer System Clock Divisor
	...
	// 2) select the oscillator source
	SYSCTL->RCC	&= ~SYSCTL_RCC_XTAL_FULL;   // clear XTAL field
	SYSCTL->RCC |= SYSCTL_RCC_XTAL_16MHZ;// configure for 16 MHz crystal
	SYSCTL->RCC2 &= ~SYSCTL_RCC2_OSCSRC2_FULL;// clear oscillator source field
	...
	SYSCTL->RCC2 &= ~ 0x1FC00000;  // clear system clock divider
	SYSCTL->RCC2 |= SYSDIV2_FREQ;  // configure for 80 MHz clock
  // 5) wait for the PLL to lock by polling PLLLRIS
	while((SYSCTL->RIS&SYSCTL_RIS_PLLLRIS)==0){};
  // 6) enable use of PLL by clearing BYPASS
	SYSCTL->RCC |= 0x00400000;
	SYSCTL->RCC2 &= ~SYSCTL_RCC2_BYPASS2;
}
